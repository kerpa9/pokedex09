...
import { less, plus } from "../store/slices/paginateSilice";
import "../components/styles/paginate.css";

const Paginate = ({ total }) => {
  const paginate = useSelector((store) => store.paginate);
  const dispatch = useDispatch();

  const handlePlus = (num) => {
    if (paginate < total) dispatch(plus(num));
  };
  const handleLess = (num) => {
    if (paginate > 1) dispatch(less(num));
  };

  return (
    <div className="paginate_header">
      <nav>
        <button className="paginate_btn1" onClick={() => handleLess(1)}>
          <svg
            viewBox="0 0 24 24"
            fill="none"
        ...
        <span className="paginate_span1">{paginate}...</span>
        <span className="paginate_span2">{total}</span>
        <button className="paginate_btn2" onClick={() => handlePlus(1)}>
          <svg
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
